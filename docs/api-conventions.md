...
This document is the single source of truth for **how** endpoints are shaped. It
defines URL conventions, response envelopes, error shapes, pagination, and the
expected use of HTTP status codes. The _what_ of each endpoint lives in the
OpenAPI snapshot at [`api/openapi.yaml`](api/openapi.yaml) and the live Swagger
UI at `http://localhost:3000/api/docs`.

---

## 1. URL structure

All routes are mounted under `/api/v1`. API versioning happens in the URL path,
not in headers — easier to debug, easier to reverse-proxy.
...
- **Resource paths are plural, lowercase, kebab-case.** ✅ `/contacts`,
  `/api-keys`, `/payment-methods` ❌ `/Contact`, `/api_key`, `/paymentMethod`
...
- **Idempotent operations** use `PUT` or `PATCH`. Non-idempotent operations use
  `POST`.
...
- `POST /conversations/:id/close` — state transition with side effects (archives
  the conversation, emits `conversation.closed` event).
- `POST /messages/:id/retry` — re-enqueues a failed message dispatch.

Use a sub-resource action **only** when the operation has side effects that make
a `PATCH` semantically wrong. Document the rationale in the controller.
...
`Cookie: wa-kijo.session_token=...` (or use the upcoming API-key auth in Phase
7).
...
> The `204` case bypasses the envelope by definition — there is no response body
> to wrap.
...
> `error` code and `details` shape. Don't construct error envelopes by hand in
> controllers — throw a typed exception and let the filter shape it.

---

## 4. Error code catalogue

Stable across releases. New codes are additive; renaming is a breaking change
requiring a CHANGELOG entry under "Changed".
...
For `VALIDATION_ERROR` the `details.fieldErrors` map is keyed by field path (dot
...
The cursor is the primary key of the last item in the page (typically a cuid).
It is stable across writes — adding new items at the head does not shift the
existing pages.
...
Offset pagination is **forbidden on hot endpoints** because `OFFSET N` walks the
index and degrades quickly past 100k rows.
...
The complete list of supported filters per endpoint is documented in the Swagger
spec at `/api/docs`.
...
For operations that the client may safely retry (e.g. creating a contact during
a flaky network), accept an `Idempotency-Key` header:
...
The server stores `(idempotency_key, tenant_id) -> response` for 24 hours and
returns the cached response on retry. Implementation lives in
...
- A `GET /api/v1/contacts` request from a user whose active org is `org_acme`
  returns only contacts where `organizationId = 'org_acme'`.
- A `GET /api/v1/contacts/:id` returns 404 if the contact exists but belongs to
  a different org. **Do not return 403**, which would leak the existence of the
  resource.
...
- The `v1` in `/api/v1/` represents a **major** API version. Major versions are
  introduced rarely (rough guideline: every 2+ years).
...
- Non-breaking additions — new optional fields, new endpoints, new error codes —
  ship under the same major version.
...
Buyers verify the signature using HMAC-SHA256 with the per-endpoint secret from
their webhook settings page.
...
1. Reads the raw body via Fastify hook (NestJS body parsing skipped for these
   routes).
...
4. Enqueues the event payload to BullMQ for asynchronous processing, keyed by
   the provider's event ID for idempotency.

Reject silently (`400` with empty body) on invalid signatures — verbose errors
help attackers fingerprint the verification logic. See
...
A complete `curl` of the contacts endpoints — useful when wiring an integration
test or a buyer's first SDK:
...
These conventions exist to make the API predictable. If you have a strong reason
to deviate (e.g. an integration partner mandates a non-standard shape), document
the deviation in:

- The controller comment
  (`// DEVIATION: <provider> requires <shape> because <reason>`).
