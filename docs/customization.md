...
>
> wa'kijo is opinionated, but every opinion can be overridden cleanly.
> This guide walks through the most common customisations and the patterns
> we recommend so your fork stays mergeable with future wa'kijo releases.

---

## 1. The Golden Rules

1. **Don't rename core files.** Renaming `BaseRepository`, the auth guards,
   or the modules under `apps/api/src/common/` makes it harder to merge
   updates from the base repository.
2. **Add, don't subtract, in shared packages.** New permissions, new DTOs,
   and new types belong in `packages/shared/`. Existing exports should be
   treated as a contract — extending is fine, redefining is not.
...
The `CLAUDE.md`, `CHANGELOG.md`, and ADR files should continue referring
to wa'kijo — they're the upstream artefacts you inherited and they'll keep
...
shadcn/ui components consume these variables, so a single file change
re-skins the entire app. The existing `--color-primary` is wa'kijo's brand
orange — change it before your first customer demo.
...
Email templates (`apps/api/src/modules/email/templates/`) include inline
brand colours and the wordmark. Update those before sending the first
production email.
...
Resend is the default. To switch to another provider (SES, Postmark,
Mailgun, etc.):
...
The `EmailService` consumes the provider through DI, so the rest of the
app is unchanged.
...
3. Remove the billing pages under `apps/web/src/app/(app)/orgs/[orgId]/billing/`.
4. Remove the `billing:*` permissions from `packages/shared/src/auth/permissions.ts`.
...
- **Webhook signature verification is mandatory.** Reject silently on
  invalid signatures (see [`.claude/rules/security.md`](../.claude/rules/security.md)).
...
Better Auth supports many strategies (passkeys, OAuth providers, OTP). To
enable one:

1. Install the relevant Better Auth plugin (or write one — Better Auth's
...
5. Update [`docs-site/concepts/authentication.mdx`](../docs-site/concepts/authentication.mdx)
   so customers know the new method is available.

Do **not** modify `AuthService.resolveContext()` for new auth methods —
that function is method-agnostic and operates on the resolved Better Auth
session.
...
The default model is fixed roles (`owner`, `admin`, `member`) and a
static permission catalogue. For customer- or tenant-defined roles:

1. Add a `Role` model in `packages/db/prisma/schema.prisma` with `name`,
   `organizationId`, and `permissions String[]`.
2. Update `Member.role` to be a `String?` referencing the new table (or
   keep the legacy field for backwards compatibility and add `customRoleId`).
3. Replace the `hasPermission()` lookup in
   `packages/shared/src/auth/permissions.ts` with a database lookup keyed
...
customer base genuinely needs it — most B2B SaaS products do fine with
fixed roles.
...
Switching to MySQL or MongoDB is **not** supported. If you're sure you
need a different database engine, you're not the customer wa'kijo was
built for — that's a fair conclusion to reach.
...
- Add a translation key to every user-visible string with a
  `// i18n: <key>` comment.
- Centralise English copy in `apps/web/src/lib/copy.en.ts` so the eventual
  i18n migration is mechanical.
- Plan for **right-to-left support** if you intend to ship Arabic,
  Hebrew, or Farsi locales — Tailwind's `dir-rtl` plugin makes this
  manageable.
...
The recommended pattern for "I want to do something when X happens" is
**domain events** dispatched through BullMQ:
...
- Add new jobs in **separate workflow files** rather than editing the
  shipped one. Less merge friction on upgrades.
...
If you do nothing else, keep these files in sync with upstream wa'kijo
releases:
...
release touches only the files above plus their tests and docs.
...
1. The relevant ADR in `docs/decisions/` — there may already be a "what if
   you want to deviate" section.
2. The skill files (`anthropic-skills:nestjs-prisma`,
   `anthropic-skills:nestjs-better-auth`) — these capture the patterns the
   core team uses internally.
3. [`SUPPORT.md`](../SUPPORT.md) — if all else fails, file a ticket. We're
   especially interested in customisations that lots of customers want, as
   they often graduate into the boilerplate itself.
